...
class Result{
	...
	public Result(int custno, String custname, int total) {
		this.custNo = custno;
		this.custName = custname;
		this.total = total;
	}
...
class Manager{
	...
	void init() {
		...
		void initResult() {
			...
			resultList.add(new Result(memberList.get(i).getCustno(),memberList.get(i).getCustname(), 0));
		}
	}
		void price() {
			for (int i = 0; i <resultList.size(); i++) {
				for (int j = 0; j <moneyList.size(); j++) {
					if(resultList.get(i).getCustNo()==moneyList.get(i).custno) {
						resultList.get(j).setTotal(moneyList.get(j).price+resultList.get(i).getTotal());
					}
				}
			}
		}
